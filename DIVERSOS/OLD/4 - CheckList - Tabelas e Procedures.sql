...
BEGIN
	...
	SELECT [Id_Alerta], [Nm_Alerta], [Ds_Mensagem], [Dt_Alerta], Fl_Tipo, NULL	
	...
	JOIN Alerta B ON A.Nm_Alerta = B.Nm_Alerta and B.Fl_Tipo = 1 and B.Dt_Alerta < A.Dt_Alerta
	where A.Fl_Tipo = 0
	...
	-- Busca os Alertas que não possuem CLEAR
	...
	WHERE	[Id_Alerta] = ( SELECT MAX([Id_Alerta]) FROM [dbo].[Alerta] B WITH(NOLOCK) WHERE A.[Nm_Alerta] = B.[Nm_Alerta] )
			AND [Nm_Alerta] NOT IN ('SQL Server Reiniciado nas últimas 24 Horas',
									'Jobs que Falharam nas últimas 24 Horas',
									'Databases sem Backup',
									'Alerta_Servidor_Cluster_Down',
									'Alerta Erros BD',
									'Banco de dados corrompido',
									'Pagina de dados corrompida',
									'Configurar Mirror',
									'Database Criada')
			AND [Fl_Tipo] = 1	-- ALERTA
