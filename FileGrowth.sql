...
SET NOCOUNT ON
	...
	-- OBS!!!!! Garanta que seu SQL Server tem o recuros IFI habilitado. 
	-- Caso contrário, o crescimento do MDF pode ficar lento.
	...
	*/
	...
	WHERE
		...
		OR
		((SF.growth/128.0) = 1 AND (SF.status & 0x100000 = 0))
		OR
		(SF.growth = 0)
		)
	...
	BEGIN 
		...
		BEGIN  
			PRINT	'ALTER DATABASE '+ '[' + @name + ']' +
					...
					', FILEGROWTH = ' +
					CASE @typeFile
						WHEN 0 THEN	(CASE	-- ARQUIVO DE LOG
